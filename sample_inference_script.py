...
SYSTEM_PROMPT = textwrap.dedent(
    ...
    Reply with exactly one message string, no quotes, no prefixes.
    """
).strip()
...
async def main() -> None:
    ...
    try:
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            last_echoed = result.observation.echoed_message
            last_reward = reward
            history.append(f"step={step} msg={message!r} reward={reward:.2f}")
        ...
        score = max(0.0, min(1.0, sum(rewards) / (MAX_STEPS * MAX_TOKENS * 0.1)))
